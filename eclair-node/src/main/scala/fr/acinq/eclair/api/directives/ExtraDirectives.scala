...
trait ExtraDirectives extends Directives {
  ...
  def withRequestedRemoteFunding: Directive1[Option[LiquidityAds.RequestRemoteFundingParams]] = formFields("requestRemoteFundingSatoshis".as[Satoshi].?, "remoteFundingMaxFeeSatoshis".as[Satoshi].?).tflatMap {
    case (Some(requestRemoteFunding), Some(remoteFundingMaxFee)) => provide(Some(LiquidityAds.RequestRemoteFundingParams(requestRemoteFunding, remoteFundingMaxFee)))
    case (Some(_), None) => reject(MalformedFormFieldRejection("remoteFundingMaxFeeSatoshis", "You must specify the maximum fee you're willing to pay when requesting inbound liquidity from the remote node"))
    case _ => provide(None)
  }
