 ...
 */
    ...
    case Event(open: OpenDualFundedChannel, d: DATA_WAIT_FOR_OPEN_DUAL_FUNDED_CHANNEL) =>
      ...
      Helpers.validateParamsDualFundedNonInitiator(nodeParams, d.init.channelType, open, remoteNodeId, localParams.initFeatures, remoteInit.features) match {
        ...
        case Right((channelFeatures, remoteShutdownScript)) =>
          ...
          val liquidityLease_opt = d.init.fundingContribution_opt.flatMap(_.signLease(nodeParams.privateKey, localFundingPubkey, open.fundingFeerate, open.requestFunds_opt))
          val tlvs: Set[AcceptDualFundedChannelTlv] = Set(
            upfrontShutdownScript_opt,
            Some(ChannelTlv.ChannelTypeTlv(d.init.channelType)),
            liquidityLease_opt.map { case (willFund, _) => willFund },
          ...
          val txBuilder = context.spawnAnonymous(InteractiveTxBuilder(
            ...
            liquidityPurchased_opt = liquidityLease_opt.map { case (_, lease) => LiquidityPurchased(isBuyer = false, lease) },
            wallet))
      ...
      Helpers.validateParamsDualFundedInitiator(nodeParams, remoteNodeId, d.init.channelType, localParams.initFeatures, remoteInit.features, d.lastSent, accept, d.init.requestRemoteFunding_opt) match {
        ...
        case Right((channelFeatures, remoteShutdownScript, liquidityLease_opt)) =>
          ...
          val txBuilder = context.spawnAnonymous(InteractiveTxBuilder(
            ...
            liquidityPurchased_opt = liquidityLease_opt.map(lease => LiquidityPurchased(isBuyer = true, lease)),
            wallet))
          txBuilder ! InteractiveTxBuilder.Start(self)
          ...
          case RbfStatus.NoRbf =>
            ...
            if (msg.feerate < minNextFeerate) {
              ...
              val fundingParams = d.latestFundingTx.fundingParams.copy(
                localContribution = addFunding_opt.map(_.fundingAmount).getOrElse(d.latestFundingTx.fundingParams.localContribution),
              ...
              val liquidityLease_opt = addFunding_opt.flatMap(_.signLease(nodeParams.privateKey, localFundingPubKey, msg.feerate, msg.requestFunds_opt))
              val txBuilder = context.spawnAnonymous(InteractiveTxBuilder(
                randomBytes32(),
                nodeParams, fundingParams,
              ...
              txBuilder ! InteractiveTxBuilder.Start(self)
              val toSend = Seq(
                Some(TxAckRbf(d.channelId, fundingParams.localContribution, liquidityLease_opt.map { case (willFund, _) => willFund })),
          ...
          LiquidityAds.validateLeaseRates_opt(remoteNodeId, d.channelId, fundingParams.remoteFundingPubKey, msg.fundingContribution, cmd.targetFeerate, msg.willFund_opt, cmd.requestRemoteFunding_opt) match {
            ...
            case Right(lease_opt) =>
              val txBuilder = context.spawnAnonymous(InteractiveTxBuilder(
                ...
                purpose = InteractiveTxBuilder.PreviousTxRbf(d.commitments.active.head, 0 msat, 0 msat, previousTransactions = d.allFundingTxs.map(_.sharedTx)),
                localPushAmount = d.localPushAmount, remotePushAmount = d.remotePushAmount,
                liquidityPurchased_opt = lease_opt.map(lease => LiquidityPurchased(isBuyer = true, lease)),
                wallet))
              txBuilder ! InteractiveTxBuilder.Start(self)
  ...
  when(WAIT_FOR_DUAL_FUNDING_READY)(handleExceptions {
    ...
    case Event(_: ReceiveTxInitRbf, d: DATA_WAIT_FOR_DUAL_FUNDING_READY) =>
